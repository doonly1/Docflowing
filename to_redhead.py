...
def add_seal_single(file_path):
    """对单个 docx 文件套红加印

    若文件名非数字前缀，会先另存为数字前缀文件再处理，不覆盖原文件。
    """
    ...
    if date_para and picture_name:
        try:
            ...
            text_area_width_pt = (section.page_width - section.left_margin - section.right_margin) / 12700  # EMU转pt
    ...
    #插入文号及分割线
    para = paras[0].insert_paragraph_before(style='Fangsong')
    para_fm(para,0,0,28.95,0,0,0,'C')
    wenhao = get_fawenzihao(signature)
    run_fm(para.add_run(wenhao),'仿宋', 16,0,0,0,0)
    pPr = para._p.get_or_add_pPr()
    # 底边框：红色 FF0000、单实线、粗细 12（=1.5pt）、间距 5
...
def add_seal(workdir):
    ...
    for file in files:
        add_seal_single(os.path.join(workdir, file))
