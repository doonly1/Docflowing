    ...
    _renderView: function() {
        ...
        viewEl.innerHTML =
            '<div class="kb-chat-container" id="kb-container">' +
                // 顶栏：右上按钮统一 pane-header-btn
                '<div class="kb-chat-topbar">' +
                    '<div class="kb-header-more-wrapper">' +
                        '<button onclick="WikiKnowledge.toggleMoreMenu(event)" title="更多" class="pane-header-btn">' +
                            '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="5" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="12" cy="19" r="1.5"/></svg>' +
                        '</button>' +
                        ...
                        '</div>' +
                    '</div>' +
                    '<button onclick="WikiKnowledge.showLLMSettings()" title="LLM 设置" class="pane-header-btn">' +
